...
int main(void) {
	...
	tda15.Pin = GPIO_PIN_15;
	tda15.Mode = GPIO_MODE_AF_PP;
	tda15.Pull = GPIO_NOPULL;
	tda15.Speed = GPIO_SPEED_HIGH;
	tda15.Alternate = GPIO_AF1_TIM2;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	...
	/* if(){
	 printf("You pressed the button %lu ms\r\n times.", button_press_counter);
	 printf("Program is in blinking state, please wait 9 sec.");
	 }*/
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	start++;
	if(start == 2){
		printf("You pressed the button %lu times\r\n.", button_press_counter);
	if (state == 0) {
