...
int main(void) {
	...
	tda0.Pin = GPIO_PIN_0;
	tda0.Mode = GPIO_MODE_OUTPUT_PP;
	tda0.Pull = GPIO_PULLDOWN;
	tda0.Speed = GPIO_SPEED_HIGH;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (state == 0) {
		...
		TIM2->ARR = 23331;
		HAL_TIM_Base_Start_IT(&TimHandle);
		printf("Changed to red, please wait for 6 seconds.\n");

	} else if (state == 0 && button_press_counter !=0){
			  uint16_t get_value = __HAL_TIM_GET_COUNTER(&TimHandle);
			 if(get_value =< 20000){
				 HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
				 HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_SET);
				 TIM2->ARR = 9999;
				 HAL_TIM_Base_Start_IT(&TimHandle);
			 } else if (get_value >= 20000){
				 state == 1;
			 }
	}
}
